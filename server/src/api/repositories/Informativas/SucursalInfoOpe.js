/* OPERACIONES SQL DE INFORMATIVA -- SUCURSAL */
import sql from 'mssql';

// Consultar y retornar los dispositivos registrados por número económico
const getDatosDispositivos = async (economico, responsable, tipo) => {
  ...
  if (tipo === 'Geografia') {
    query = ` 
          ...
          WHERE dispo.economico = @economico AND sucu.ingresponsable = @responsable
          `;
  } else {
    query = `
          SELECT dispo.nombre AS nombre, dispo.ip AS ip, dispo.economico AS economico, 
...
const infoGeneralDispositivo = async (ip) => {
  const request = new sql.Request();
  request.input('ip', sql.VarChar, ip);
...
const dispositivosSucursal = async (economico) => {
  const request = new sql.Request();
  request.input('economico', sql.VarChar, economico);
...
const actualizarGeneral = async (general, ip) => {
  const request = new sql.Request();
...
const actualizarDescripcion = async (descripcion, ip) => {
  const request = new sql.Request();
