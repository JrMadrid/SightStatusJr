...
const host = config.APPhost; // Define el host de la aplicación, si no está definido en la configuración, usa 'localhost' por defecto
const isDev = config.DEV; // Si estamos en modo desarrollo

export const appMiddlewares = (app) => {
  app.use(cors({
    origin: isDev ? 'http://localhost:3000' : '', // Permite solicitudes desde cualquier origen
  ...
  // Session
  app.use(session({
    secret: 'mysecretkey_statuskey', // Clave secreta utilizada para firmar la ID de la sesión. Es fundamental que esta clave sea única y difícil de adivinar para asegurar la integridad y autenticidad de la sesión del usuario.
